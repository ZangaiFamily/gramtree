...
app/
  ...
  globals.css                   # 全局样式、首页、练习页和音频检测页样式
  internal/
    page.tsx                    # 内部语法图页面
    audio-check/page.tsx        # 音频能力检测页
lib/
  grammar.ts                    # compromise 驱动的句法分析模块
  stt/                          # 听读识别 session、发音近似匹配和 provider 选择器
docs/
...
该页面复用当前听读流程的 `SRP` provider，不包含已废弃的本地模型识别逻辑。

### `lib/stt`

听读识别相关模块：

- `speechRecognitionProvider.ts`：封装浏览器原生 `SpeechRecognition` session。
- `readPracticeProvider.ts`：提供听读 provider 选择器，当前只返回 `SRP`。
- `scoring.ts`：提供文本归一化、目标匹配和近音词容错。
- `pronunciation.ts`：维护轻量发音等价规则，用于修正 `in` 被识别成 `yin`、`sing` 等近音词的情况。
...
当前 STT 依赖浏览器原生 `SpeechRecognition`，因此会受到浏览器兼容性和近音词识别偏差影响。代码层通过 `samePronunciation()` 给少量高频近音词做容错，降低单词卡片跟读误判。
